...
namespace CU
{
	constexpr auto PID_MAX = std::numeric_limits<pid_t>::max();

	class SchedAffinity
	{
		public:
			static CU_INLINE SchedAffinity FromTask(pid_t pid) noexcept
			{
				cpu_set_t cpuset{};
			...
			CU_INLINE void toTask(pid_t pid) const noexcept
			{
	...
	CU_INLINE void SetTaskSchedPrio(pid_t pid, int prio) noexcept
	{
		if (prio <= 99 && prio >= 1) {
	...
	CU_INLINE int GetTaskSchedPrio(pid_t pid) noexcept
	{
